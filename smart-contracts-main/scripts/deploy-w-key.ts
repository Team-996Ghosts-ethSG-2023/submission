...
if (process.env.PK && process.env.SPONSOR_PK && process.env.MAKER_PK && process.env.USER_PK){
    ...
    await token.mint(sponsor.address, taskAmount, {gasLimit:2100000 , gasPrice: 50000000000});
    ...
    await campaign.grantSponsor(sponsor.address, {gasLimit:2100000 , gasPrice: 50000000000});
    await campaign.grantMaker(maker.address, {gasLimit:2100000 , gasPrice: 50000000000});
    console.log("grant permission");
  
    await token.connect(sponsor).increaseAllowance( await campaign.getAddress(),taskAmount, {gasLimit:2100000 , gasPrice: 50000000000});
    console.log("sponsor's allowance increased for them to be able to submit erc20 reward later.")
        ...
        await campaign.connect(sponsor).addNewTask(tokenAddress,campaignDescription, taskAmount, rewardPerCompletedTask, {gasLimit:2100000 , gasPrice: 50000000000});
        console.log("added new task")
        const taskId =0;
        const mockIPFSLink="mockIPFSLink";
        await campaign.connect(user).addNewCompletedTask(taskId, mockIPFSLink, {gasLimit:2100000 , gasPrice: 50000000000});
        console.log("completed new task")

        await campaign.connect(maker).approveCompletedTask(taskId, user.address, {gasLimit:2100000 , gasPrice: 50000000000});
